...
try{
  let health;
  for(let i=0;i<20;i++){try{health=await fetch(`http://127.0.0.1:${port}/health`);if(health.ok)break}catch{}await wait(100)}
  ...
  const secret=await fetch(`http://127.0.0.1:${port}/server.js`);assert.equal(secret.status,404);
